...
Token Scanner::get_next_token() noexcept
{
    ...
    {
        this->unget();
        return this->build_word_token();
    }

    switch (c)
    {
    case '-':
        return std::make_pair(TokenType::Neg, "-");
    case '+':
    ...
    return std::make_pair(TokenType::Unknown, std::string{c});
}
...
Token Scanner::build_word_token() noexcept
{
    std::string value{};
    ...
    TokenType token_type = from_string(value);

    return std::make_pair(token_type, value);
}
